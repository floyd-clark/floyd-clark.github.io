...
function renderDecisionPage_(parameters) {
  ...
  return HtmlService.createHtmlOutput(`<!doctype html><html><head><base target="_top"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover"><meta name="theme-color" content="#071019"><title>Review access request</title><style>
    *{box-sizing:border-box}html{color-scheme:dark}body{min-height:100vh;margin:0;background:#071019;color:#ecf4fa;font:16px/1.5 system-ui,-apple-system,sans-serif}main{width:min(100%,560px);min-height:100vh;margin:0 auto;padding:max(28px,env(safe-area-inset-top)) max(20px,env(safe-area-inset-right)) max(28px,env(safe-area-inset-bottom)) max(20px,env(safe-area-inset-left));background:#0b1621}small{color:#8da1b1}h1{margin:18px 0 6px;font-size:clamp(28px,8vw,38px);line-height:1.08}strong{display:block;margin:28px 0 8px;font-size:clamp(20px,5.8vw,27px);word-break:break-word}.hint{color:#aebdca}.actions{display:grid;gap:12px;margin-top:30px}button{width:100%;min-height:58px;padding:14px 20px;border:0;border-radius:14px;font:800 18px/1 system-ui,-apple-system,sans-serif;cursor:pointer;touch-action:manipulation}.approve{background:#5cc8ff;color:#04111b}.deny{background:#263746;color:#ecf4fa}@media(min-width:560px){main{min-height:0;margin:8vh auto;padding:38px;border:1px solid #294153;border-radius:22px}.actions{grid-template-columns:1.35fr .65fr}}</style></head><body><main><small>Signal Intelligence · ${requestId}</small><h1>Approve this reviewer?</h1><p class="hint">Review the email address, then approve or deny from this phone or computer.</p><strong>${email}</strong><p>Current status: ${currentStatus}</p><form method="post" action="${serviceUrl}"><input type="hidden" name="token" value="${escapeHtml_(token)}"><div class="actions"><button class="approve" name="decision" value="approved">Approve access</button><button class="deny" name="decision" value="denied">Deny</button></div></form></main></body></html>`)
